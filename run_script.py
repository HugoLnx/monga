...
def clear():
	os.system("sudo rm -rf " + os.path.join(BUILD, '*'))
	...
	for file in os.listdir("./tests"):
		if file.endswith(".in") or file.endswith(".rejected"):
			...
			if error_msg:
				...
				print
				fails += 1
	print "Result: " + (len(list_files) - fails) + " of " + len(list_files) + " test cases passed."
...
def execute_content(content):
	os.system(os.path.join(TEST_DIST,"main") + "<<EOF" + content + "EOF")
	return
...
def execute_reject_test(test_path):
	...
	for line in file_test_path:
		...
		if is_rejected(test_output):
			msgs.append(content + " should be rejected but the output was: " + test_output + "".strip())
	if len(msgs) == 0:
		return None
	else:
		return "\n".join(msgs)
...
def failing_msg(output, expected):
	if expected:
		for index,line in enumerate(expected):
			if line != output.readlines()[index]:
				return "Difference on line " + index + ": \
						Expected: " + (line.strip() if line else "<no-line>") + \
						"Output: " + str(output.readlines()[index].strip()) + "".strip()
		return None
	else:
